...
| 3.x   | Departments, staff assignment, multi-hospital ops  | Partial (4.2–4.4)      |
| 4.1   | Hospital Administration architecture               | Done                   |
| 4.2   | Department management                              | Done                   |
| 4.3   | Staff management                                   | Done                   |
| 4.4   | Staff assignment                                   | Done                   |
| 4.5   | User invitation                                    | Done                   |
| 4.6   | User management                                    | Done                   |
| 4.7   | Hospital Administration UI                         | Done                   |
...
## Phase 4.1 — Hospital Administration architecture

### Objective

Design the Hospital Administration domain foundation: organization package, staff
hierarchy contracts, and shared employment enums — without Departments, Doctors,
Nurses, or APIs.

### What was done

- Top-level `com.healthcare.hms.organization` module
- Abstract `Staff` `@MappedSuperclass` (tenant-owned, UUID, audit, soft delete)
- Enums: `EmploymentStatus`, `EmploymentType`, `DepartmentType`
- Reserved `department` and `staff` subpackages for later concrete types
- Architecture doc with package structure, ER diagram, module interaction diagram
...
- Organization package + Staff base + enums delivered; no premature clinical staff APIs
...
Implement tenant-isolated department CRUD with uniqueness, soft delete, search,
pagination, sorting, and filtering.

### What was done

- `Department` entity (`TenantOwnedEntity`) + `DepartmentStatus`
- DTOs, MapStruct mapper, Bean Validation
- Repository + Specifications for search/filter
- Service with uniqueness, head-user tenant check, audit, soft-delete key release
- REST controller `/api/v1/departments`
- Flyway `V12__departments.sql`
- `HospitalQueryService` for cross-module hospital lookup
- `PageResponse` common pagination envelope
...
- Hospital admins can create/list/update/soft-delete departments within their tenant

---

## Phase 4.3 — Staff management

### Objective

Administer hospital personnel profiles (Doctor, Nurse, Receptionist, Laboratory Staff,
Pharmacist) linked to Users and Departments with RBAC, tenant isolation, and audit.

### What was done

- Five concrete `Staff` entities + Flyway `V13__staff_profiles.sql`
- CRUD APIs under `/api/v1/doctors|nurses|receptionists|laboratory-staff|pharmacists`
- Role-matched user validation; department required; unique employee codes / licenses
- `STAFF_*` permissions for operational staff; `DOCTOR_*` for doctors
- Cross-type membership guard (one employment profile per user)
- Soft delete with unique-key release; audit logging

### Definition of Done (met)

- Hospital admins can manage all five staff profile types within their tenant

---

## Phase 4.4 — Staff assignment

### Objective

Assign and transfer staff across departments, designate department heads, prevent
duplicate affiliations, and retain assignment history — with transactions, audit,
validation, and tenant isolation.

### What was done

- `StaffDepartmentAssignment` history aggregate + Flyway `V14__staff_department_assignments.sql`
- Unique open assignment per staff (`open_slot` generated column)
- Department `head_staff_id` / `head_staff_type` (synced with `head_user_id`)
- APIs under `/api/v1/staff-assignments` and `/api/v1/departments/{id}/head`
- `StaffProfileDirectory` for polymorphic staff resolution
- History also written from staff create/update/soft-delete
- Transfer clears department-head role when the head leaves
...
- Hospital admins can assign/transfer staff, set department heads, and query history within their tenant

---

## Phase 4.5 — User invitation

### Objective

Hospital admins invite users by email; invitees accept (create account + role + join
hospital) or reject; admins can resend or cancel pending invitations.

### What was done

- `UserInvitation` entity + Flyway `V15__user_invitations.sql`
- Hashed single-use tokens with configurable TTL (default 72h)
- `InvitationEmailService` abstraction over `EmailSender`
- Admin APIs: create / list / get / resend / cancel
- Public APIs: token preview / accept / reject
- Accept creates `User` (ACTIVE, email verified), assigns tenant role, links `accepted_user_id`
- Duplicate pending invite + existing-user email conflicts blocked
- Audit actions: `INVITATION_SENT|ACCEPTED|REJECTED|CANCELLED|RESENT`
...
- Hospital admins can invite users; invitees can accept/reject; resend/cancel work for pending invites
...
Tenant-scoped user directory with search/filter/pagination/sorting and status-based
lifecycle (activate, deactivate, suspend, restore) — no physical deletion.

### What was done

- `UserStatus.SUSPENDED` + transition helpers on `User`
- Admin APIs under `/api/v1/users`
- Specifications for `q`, `status`, `roleType`, `emailVerified`
- Profile update (name/phone); session revoke on deactivate/suspend
- Self-status-change forbidden; audit on mutations
- RBAC: `USER_READ` / `USER_UPDATE`
...
- Hospital admins can view/search/filter users and manage status without hard deletes

---

## Phase 4.7 — Hospital Administration UI

### Objective

Ship the permission-aware Hospital Administration frontend for Phase 4 APIs:
dashboard overview, departments, staff directory, user management, and invitations.
Patients and appointments remain out of scope.

### What was done

- Feature module `frontend/src/features/hospital-admin`
- Routes: `/app` (admin dashboard), `/app/departments`, `/app/staff`, `/app/users`,
  `/app/invitations`
- TanStack Query for server state; Redux Toolkit UI slice for list filters/pagination
- React Hook Form + Zod dialogs; shadcn table/select/dialog/tabs
- Search, filters, pagination, responsive layout, `<Can>` permission rendering
- Navigation + fail-closed route catalog updated (patients/appointments still coming soon)
...
## Phase 4.8 — Hospital Administration security review

### Objective

Complete security review of Hospital, Department, User, and Invitation APIs covering
tenant isolation, RBAC, privilege escalation, cross-tenant access, audit logs, sensitive
data exposure, and broken authorization — then remediate until no Critical issues remain.
...
- No Critical security issues remain in Hospital / Department / User / Invitation APIs
...
End-to-end production readiness review of Hospital Administration (Phases 4.1–4.8):
compilation, Flyway, Docker contract, Next.js build, tenant/RBAC, department/staff/user/
invitation flows, Soft Architecture conformance — remediate until no Critical or High
...
- No Critical or High severity Hospital Administration issues remain
- Accept → account → assign → deactivate → restore lifecycle is complete in FE+BE
...
| `V12__departments.sql`                           | 4.2   | Departments table                                                         |
| `V13__staff_profiles.sql`                        | 4.3   | Staff tables + STAFF_* permissions                                        |
| `V14__staff_department_assignments.sql`          | 4.4   | Assignment history + department head staff FK                             |
